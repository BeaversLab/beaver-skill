...
| GitHub CLI (gh) | 命令行工具 | 否 | 用于识别第三方贡献者和查看 CI 状态。 |

## 注意事项

- **提交规范**：建议遵循 Conventional Commits（如 `feat: ...`, `fix: ...`），否则技能可能无法准确识别变更类型。
- **破坏性变更**：如果包含破坏性变更，请在提交信息中包含 `BREAKING CHANGE` 字样以触发主版本号更新提醒。
- **远端权限**：执行推送（Push）操作需要当前环境具备 Git 远端仓库的写入权限。

## 常见问题

**Q：它如何知道我的 Changelog 文件在哪？**
**A：** 技能会自动扫描根目录下以 `CHANGELOG` 或 `HISTORY` 开头的 Markdown 文件，并根据后缀（如 `.en.md`）识别语言。

**Q：我可以自定义更新日志的分类吗？**
**A：** 可以通过在项目根目录创建 `.releaserc.yml` 文件来自定义 conventional commit 类型与 Changelog 章节的映射关系。

**Q：如果不安装 GitHub CLI 还能用吗？**
**A：** 可以。GitHub CLI 仅用于增强功能（如识别 PR 作者和 CI 状态），缺失时不影响核心发布流程。
